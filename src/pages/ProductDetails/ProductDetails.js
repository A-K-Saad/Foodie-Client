...
const ProductDetails = ({ setCartUpdate }) => {
  ...
  useEffect(() => {
    fetch(`https://foodie-mart-aks.onrender.com/products/${productId}`)
      .then((res) => res.json())
      .then((data) => {
        setIsLoading(false);
        setProduct(data);
    ...
    fetch(
      `https://foodie-mart-aks.onrender.com/products/category/${product?.category}`
    )
      .then((res) => res.json())
  ...
  const handleAddToCart = () => {
    if (user?.email) {
      fetch("https://foodie-mart-aks.onrender.com/carts", {
        method: "POST",
        headers: {
    ...
    <>
          ...
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            ...
            >
              <img
                src={product?.photo}
                alt={product?.name}
                className="max-h-full w-auto md:h-56"
              />
            </div>
      ...
      <div className="text-center">
        <div className="bg-white px-8 md:px-32">
          <ProductReviews
            productId={productId}
            toShowForm={user?.email}
          ></ProductReviews>
        </div>
        {sameProducts && (
          <ProductCarousel
            products={sameProducts}
            header="Same Products"
          ></ProductCarousel>
        )}
      </div>
    </>
