...
const CartCanvas = ({ cartUpdate }) => {
  ...
  useEffect(() => {
    if (user?.email) {
      fetch(`https://foodie-mart-aks.onrender.com/carts/${user?.email}`)
        .then((res) => res.json())
  ...
  return (
    <>
      <div className="fixed top-0 right-0 bottom-0 m-auto flex items-center z-[49] pointer-events-none">
        <div
          className="bg-white shadow rounded-l-md p-2 md:p-5 text-center cursor-pointer pointer-events-auto"
          onClick={() => setIsCartOpen(true)}
        >
          <i className="fas fa-shopping-basket text-xl md:text-2xl text-gray-800"></i>
          <h1 className="font-black text-gray-700 text-sm">
